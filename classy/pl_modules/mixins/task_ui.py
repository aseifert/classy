...
try:
    ...
    class TokenTaskUIMixin(TaskUIMixin):
        ...
        color_mapping = collections.defaultdict(
            lambda: TokenTaskUIMixin.color_generator()
        )
    ...
    class MockTaskServeMixin(TaskUIMixin):
        def ui_render_task_in_sidebar(self):
            raise ImportError(
                f"UI mixins could not be properly instantiated as one of its requirements ([streamlit, st-annotated-text]) are not isntalled. To fix this issue, run `pip install {get_optional_requirement('streamlit')} {get_optional_requirement('st-annotated-text')}`"
            )

        def ui_read_input(
            self, inference_message: str, inferred_examples: List[ClassySample]
        ) -> Optional[ClassySample]:
        ...
        def ui_render(self, predicted_sample: ClassySample, time: float):

            raise ImportError(
                f"UI mixins could not be properly instantiated as one of its requirements ([streamlit, st-annotated-text]) are not isntalled. To fix this issue, run `pip install {get_optional_requirement('streamlit')} {get_optional_requirement('st-annotated-text')}`"
            )

    GenerationTaskUIMixin = MockTaskServeMixin
    QATaskUIMixin = MockTaskServeMixin
    SentencePairTaskUIMixin = MockTaskServeMixin
    SequenceTaskUIMixin = MockTaskServeMixin
    TokenTaskUIMixin = MockTaskServeMixin
